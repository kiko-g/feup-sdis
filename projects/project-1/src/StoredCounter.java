...
public class StoredCounter implements Callable<String> {
    int port;
    int chunkNo;
    int counter;
    int peerId;
    ...
    StoredCounter(int port, String IP, String fileID, int chunkNo, int peerId, int replicationDeg) {
        this.IP = IP;
        this.port = port;
        this.fileID = fileID;
            ...
            if(header.getFileId().equals(this.fileID) && header.getChunkNo() == this.chunkNo) {
                if(header.getMessageType().name().equals(MessageType.STORED.name())) {
                    if (header.getSenderId() != peerId)
                        ...
                        this.counter++;
                    if(this.counter >= this.replicationDeg)
