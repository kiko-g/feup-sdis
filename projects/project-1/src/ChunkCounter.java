...
public class ChunkCounter implements Callable<String> {
    int port;
    int chunkNo;
    int counter=0;
    String IP;
    ...
    ChunkCounter(int port, String IP, String fileID, int chunkNo) {
        this.IP = IP;
        this.port = port;
        this.fileID = fileID;
    ...
    Callable<String> chunkCounterTask = () -> {
        ...
        while(true) {
            ...
            if(header.getFileId().equals(this.fileID) && header.getChunkNo() == this.chunkNo) {
                if(header.getMessageType().name().equals(MessageType.GETCHUNK.name())) {
                    this.counter++;
                }
            }
